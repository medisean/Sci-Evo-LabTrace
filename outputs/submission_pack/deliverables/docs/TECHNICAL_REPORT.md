...
当前版本以赛事给出的从头设计人工荧光素酶为种子案例，并扩展到机器学习组合文库设计、定向进化复盘、LLM 辅助工业抗体耐受性优化、酵母分泌信号工程等子方向。每条 case 都覆盖初始目标定义、关键决策、工具或实验、观测结果、迭代逻辑与最终指标确认。
...
本项目使用 MinerU API 作为科学文献解析工具链的核心组件。5 条 gold case 对应 PDF 均已完成真实解析，解析产物包括：
...
- `data/interim/mineru/Sci-Evo-Sample/model.json`
- `data/interim/mineru/SELT-PROT-0002/full.md`
- `data/interim/mineru/SELT-PROT-0003/full.md`
- `data/interim/mineru/SELT-PROT-0004/full.md`
- `data/interim/mineru/SELT-PROT-0005/full.md`
- 对应 `content_list.json`、`content_list_v2.json`、`layout.json`、`model.json` 和图片目录。
...
此外，仓库增加了四个面向材料完整度冲刺的质量控制文件或自动检查：
...
- 5 条 gold case（1 条赛事样例 + 4 条 CC-BY 开放论文案例）。
- 29 个科研轨迹步骤和 63 条自动生成评测任务。
...
当前版本已经超过“至少 3 条完整 gold case”的基础完整度门槛，并且 5 条 case 均已补齐本地 MinerU 解析记录。剩余工作主要是最终提交材料整理，而不是数据结构本身。下一步优先顺序是：

1. 在最终整理前用 `git status --short` 确认工作区干净，并保留明确提交记录。
2. 录制 3-5 分钟材料介绍视频，重点展示数据 schema、强 case、MinerU 解析和质量报告。
3. 对新增开放论文使用 `scripts/mineru_parse.py` 和 token 文件补齐本地 MinerU 解析产物。
4. 如需进一步冲击更高排名，从 `reports/VETTED_SOURCE_QUEUE.md` 继续扩展更多 `permitted_for_local_processing` 的开放论文。
5. 重新生成数据集、评测任务、质量报告和完整性检查报告，并导出最终技术报告/PDF。
